...
class TouristAttractionServiceTest {
    ...
    void testAddAttractionCallsRepository() {
        TouristAttraction attraction = new TouristAttraction(
                ...
                City.København,
                List.of(Tags.Gratis, Tags.Historie)
        );
    ...
    void testDeleteAttractionByName() {
        ...
        when(repository.findByName("Ribe Vikinge Center")).thenReturn(attraction);

        service.deleteAttraction("Ribe Vikinge Center");

        verify(repository, times(1)).findByName("Ribe Vikinge Center");
    ...
    void testUpdateAttractionByName() {
        ...
        when(repository.findByName("Aalborg Zoo")).thenReturn(existing);

        service.updateAttraction("Aalborg Zoo", updated);

        verify(repository, times(1)).findByName("Aalborg Zoo");
